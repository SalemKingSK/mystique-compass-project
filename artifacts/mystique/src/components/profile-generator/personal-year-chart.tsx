...
import { getCompoundForPYN, getClassicCompoundForPYN } from '../../lib/numerology/chaldean-pyn-compounds';
import { computeRawPersonalYear, computeRawPersonalYearClassic, reduceNum as reduceMasterSafe } from '../../lib/numerology/personal-year-full';
...
export const PersonalYearChart: React.FC<PersonalYearChartProps> = ({
  ...
  const { fullData, currentYearIdxInFull } = useMemo(() => {
    ...
    for (let i = -HALF_RANGE; i <= HALF_RANGE; i++) {
      ...
      // doesn't exist.
      const powerKey = (pyn === 11 ? 2 : pyn === 22 ? 4 : pyn === 33 ? 6 : pyn) as keyof typeof POWER_LEVELS;
      ...
      const rawClassic = computeRawPersonalYearClassic(birthDay, birthMonth, displayYear);
      const pynClassic = reduceMasterSafe(rawClassic);
      const cpdClassic = getClassicCompoundForPYN(birthDay, birthMonth, displayYear);

      fullData.push({
        year: displayYear,
        pyn,
        power,
        meaning: PERSONAL_YEAR_MEANINGS[powerKey as keyof typeof PERSONAL_YEAR_MEANINGS] || '',
        ...(cpd ? {
