...
import { TemporalPredictionDisplay, TemporalPredictionPanel } from './temporal-prediction-display';
import { PsychomatrixDisplay } from './psychomatrix-display';
import { SynthesisAstroSections, SynthesisAstroMoreSections, SynthesisNumerologySections, SynthesisPsychomatrixSections, SynthesisFourPlanesSections, SynthesisCellPairsSections, NumberMeaningsSection, useSynthesis } from './synthesis-sections';
import {
ArrowLeft, History, Heart, Home, Users, Briefcase,
AlertTriangle, Brain, ChevronDown, BookUser,
Sparkles, Info, AlertOctagon, Star, ChevronUp,
...
import { generateEpub } from './epub-export';
...
1: [
{ startMonth:7,  startDay:21, endMonth:8,  endDay:28, label:'House of the Sun'       },
{ startMonth:3,  startDay:21, endMonth:4,  endDay:28, label:'Vernal Equinox'          },
],
2: [
{ startMonth:6,  startDay:20, endMonth:7,  endDay:27, label:'House of the Moon'       },
],
3: [
{ startMonth:2,  startDay:19, endMonth:3,  endDay:27, label:'House of Jupiter (Neg.)' },
{ startMonth:11, startDay:21, endMonth:12, endDay:27, label:'House of Jupiter (Pos.)' },
],
4: [
{ startMonth:6,  startDay:21, endMonth:7,  endDay:27, label:'Moon Period'             },
{ startMonth:7,  startDay:22, endMonth:8,  endDay:31, label:'Sun Period'              },
],
5: [
{ startMonth:5,  startDay:21, endMonth:6,  endDay:27, label:'House of Mercury (Pos.)' },
{ startMonth:8,  startDay:21, endMonth:9,  endDay:27, label:'House of Mercury (Neg.)' },
],
6: [
{ startMonth:4,  startDay:20, endMonth:5,  endDay:27, label:'House of Venus (Pos.)'   },
{ startMonth:9,  startDay:21, endMonth:10, endDay:27, label:'House of Venus (Neg.)'   },
],
7: [
{ startMonth:6,  startDay:21, endMonth:7,  endDay:27, label:'House of the Moon'       },
],
8: [
{ startMonth:12, startDay:21, endMonth:1,  endDay:26, label:'House of Saturn (Pos.)'  },
{ startMonth:1,  startDay:26, endMonth:2,  endDay:26, label:'House of Saturn (Neg.)'  },
],
9: [
{ startMonth:3,  startDay:21, endMonth:4,  endDay:26, label:'House of Mars (Pos.)'    },
{ startMonth:10, startDay:21, endMonth:11, endDay:27, label:'House of Mars (Neg.)'    },
],
};

function isInStrongPeriod(psychicNum: number, month: number, day: number): DateRange | null {
...
warning: 0, caution: 1, opportunity: 2, info: 3,
};
...
// ── Current age ────────────────────────────────────────────────────────────
const birthThisYear = new Date(today.getFullYear(), numerology.birthMonth - 1, numerology.birthDay);
const hadBirthday   = today >= birthThisYear;
const currentAge    = today.getFullYear() - (numerology.birthYear ?? 1900) - (hadBirthday ? 0 : 1);
const nextAge       = currentAge + 1;

// ── 1. Personal Year ───────────────────────────────────────────────────────
if (py === 4) {
alerts.push({
id: 'py4', severity: 'warning',
title: 'Personal Year 4 — Foundation Cycle',
message: 'The Consolidation Cycle demands patience and structural work. Restriction and forced slowing are the hallmarks of this year. Prioritise foundations, practical systems, and long-term building over ambition and outward expansion. Great effort now is the seed of Year 5 freedom.',
});
}
if (py === 7) {
alerts.push({
id: 'py7', severity: 'warning',
title: 'Personal Year 7 — Retreat & Sacrifice',
message: 'An inward, spiritual year of deep reflection and solitude. Avoid major financial or relational decisions — the cosmos requests retreat, not expansion. Trust that inner development now yields substantial outer rewards in the powerful Year 8 that follows.',
});
}
if (py === 9) {
alerts.push({
id: 'py9', severity: 'caution',
title: 'Personal Year 9 — Great Completion',
message: 'The Great Completion cycle brings endings, release, and culmination. Resist clinging to what is already departing from your life. Radical release this year clears the path and unlocks the full creative power of the incoming Year 1 cycle.',
});
}
...
// ── 4. Climacteric years ────────────────────────────────────────────────
const isCurrentClimax = currentAge > 5 && climReducers.includes(reducePsychic(currentAge));
const isNextClimax    = nextAge > 5    && climReducers.includes(reducePsychic(nextAge));

if (isCurrentClimax) {
alerts.push({
id: 'climax-current', severity: 'opportunity',
...
: '';
alerts.push({
id: 'strong-period', severity: 'opportunity',
...
alerts.push({
id: 'double-power', severity: 'opportunity',
...
const upcomingDates = allLuckyDates.filter(d => d > todayDate + 1).slice(0, 4);
if (upcomingDates.length > 0) {
const dateList = upcomingDates.map(ordinal).join(', ');
alerts.push({
id: 'lucky-dates', severity: 'info',
...
// ── Alert config ──────────────────────────────────────────────────────────────
const ALERT_CONFIG: Record<CosmicAlert['severity'], {
bg: string; borderColor: string; icon: string;
badgeBg: string; badgeColor: string; textColor: string;
}> = {
warning: {
...
// ── Single Toast popup ────────────────────────────────────────────────────────
function CosmicToast({ alert, onDismiss }: { alert: CosmicAlert; onDismiss: () => void }) {
...
style={{
width: '100%', maxWidth: 360, padding: '0.65rem 0.8rem',
borderRadius: '0.75rem', background: cfg.bg,
border: `1px solid ${cfg.borderColor}`,
backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
boxShadow: `0 8px 32px rgba(0,0,0,0.45), 0 0 0 1px ${cfg.borderColor}`,
display: 'flex', gap: '0.65rem', alignItems: 'flex-start',
pointerEvents: 'all', cursor: 'pointer', position: 'relative', overflow: 'hidden',
}}
onClick={onDismiss}
role="alert"
>
...
style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, background: cfg.badgeColor, opacity: 0.45, transformOrigin: 'left' }}
/>
<span style={{ fontSize: '1rem', flexShrink: 0, lineHeight: 1.3, filter: 'drop-shadow(0 0 4px currentColor)' }}>{cfg.icon}</span>
<div style={{ flex: 1, minWidth: 0 }}>
<p style={{ margin: '0 0 0.2rem', fontSize: '0.58rem', fontFamily: "\'Cinzel\',serif", fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: cfg.badgeColor }}>
{alert.title}
</p>
<p style={{ margin: 0, fontSize: '0.65rem', color: cfg.textColor, lineHeight: 1.55, fontFamily: "\'Cormorant Garamond\',serif" }}>
{alert.message}
</p>
</div>
<button
onClick={(e) => { e.stopPropagation(); onDismiss(); }}
style={{ background: 'none', border: 'none', padding: '0.1rem', cursor: 'pointer', color: cfg.badgeColor, opacity: 0.5, fontSize: '0.9rem', lineHeight: 1, flexShrink: 0 }}
aria-label="Dismiss"
>×</button>
</motion.div>
);
...
function CosmicAlertsArchive({ seen }: { seen: CosmicAlert[] }) {
const [open, setOpen] = React.useState(false);
if (seen.length === 0) return null;

const BADGE_LABELS: Record<CosmicAlert['severity'], string> = {
warning: 'Warning', caution: 'Caution', opportunity: 'Opportunity', info: 'Info',
};
...
justifyContent: 'space-between', padding: '0.8rem 1rem',
background: 'none', border: 'none', cursor: 'pointer', gap: '0.6rem',
textAlign: 'left',
}}
>
...
<div>
<div style={{ fontFamily: "\'Cinzel\',serif", fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(212,175,55,0.88)' }}>
Cosmic Alerts
</div>
<div style={{ fontSize: '0.55rem', color: 'rgba(200,180,240,0.4)', fontStyle: 'italic', marginTop: '0.08rem' }}>
{seen.length} personalised signal{seen.length !== 1 ? 's' : ''} · tap to read
</div>
</div>
</div>
<ChevronDown
style={{
color: 'rgba(212,175,55,0.4)', width: 15, height: 15, flexShrink: 0,
...
key={a.id}
style={{
padding: '0.65rem 0.8rem', borderRadius: '0.7rem',
background: cfg.bg, border: `1px solid ${cfg.borderColor}`,
display: 'flex', gap: '0.6rem', alignItems: 'flex-start',
}}
>
<span style={{ fontSize: '0.95rem', flexShrink: 0, lineHeight: 1.3 }}>{cfg.icon}</span>
<div style={{ flex: 1, minWidth: 0 }}>
<div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
<span style={{
fontSize: '0.45rem', fontFamily: "\'Cinzel\',serif", letterSpacing: '0.1em',
...
}}>{BADGE_LABELS[a.severity]}</span>
<p style={{ margin: 0, fontSize: '0.6rem', fontFamily: "\'Cinzel\',serif", fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: cfg.badgeColor }}>
{a.title}
</p>
</div>
<p style={{ margin: 0, fontSize: '0.67rem', color: cfg.textColor, lineHeight: 1.62, fontFamily: "\'Cormorant Garamond\',serif" }}>
{a.message}
</p>
</div>
</div>
...
function PersonalizedAlerts({ alerts }: { alerts: CosmicAlert[] }) {
...
const t = setTimeout(() => { setStarted(true); setQueue([...alerts]); }, ALERT_DISPLAY_DELAY);
return () => clearTimeout(t);
}, [alerts]);

React.useEffect(() => {
if (!started || visible !== null || queue.length === 0) return;
timerRef.current = setTimeout(() => { setVisible(queue[0]); }, ALERT_STAGGER);
...
return (
<>
<CosmicAlertsArchive seen={seen} />
<div
style={{
position: 'fixed', bottom: '5.5rem', left: '50%',
transform: 'translateX(-50%)', width: 'calc(100% - 2rem)',
maxWidth: 400, zIndex: 200, pointerEvents: 'none',
...
{visible && (
<CosmicToast key={visible.id} alert={visible} onDismiss={dismissCurrent} />
)}
</AnimatePresence>
</div>
</>
...
function ExportFab({ insight, numerology }: { insight: AstroInsightOutput; numerology: NumerologyData }) {
const [open, setOpen] = React.useState(false);
...
toast({ variant: 'destructive', title: 'EPUB Export Failed', description: 'Could not generate the EPUB. Please try again.' });
}
finally { setBusy(null); }
};
...
style={{
position: 'fixed', bottom: '108px', right: '16px', zIndex: 49,
...
background: 'rgba(9,16,35,0.97)', border: '1px solid rgba(52,211,153,0.4)',
borderRadius: '12px', padding: '10px 16px', color: '#34d399',
...
style={{
display: 'flex', alignItems: 'center', gap: '10px',
background: 'rgba(9,16,35,0.97)', border: '1px solid rgba(212,175,55,0.4)',
borderRadius: '12px', padding: '10px 16px', color: '#d4af37',
fontFamily: "'Cinzel', serif", fontSize: '0.6rem', letterSpacing: '0.2em',
textTransform: 'uppercase', cursor: 'pointer',
backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
boxShadow: '0 8px 32px rgba(0,0,0,0.5)', whiteSpace: 'nowrap',
...
<div className="fixed z-50 pointer-events-auto" style={{ bottom: '68px', right: '24px' }}>
<Button
...
// ── Floating Nav ──────────────────────────────────────────────────────────────
function FloatingNavigation({ onReset, onHistoryOpen, insight, numerology }: { onReset: () => void; onHistoryOpen: () => void; insight: AstroInsightOutput; numerology: NumerologyData }) {
return (
<>
...
{/* ── Personalised Alerts ── */}
{alerts.length > 0 && <PersonalizedAlerts key={`${insight.name}-${numerology.birthDay}-${numerology.birthMonth}-${numerology.birthYear}`} alerts={alerts} />}

<CosmicProfilerPanel insight={insight} numerology={numerology} />
<TemporalPredictionPanel birthDay={numerology.birthDay} birthMonth={numerology.birthMonth} birthYear={numerology.birthYear || new Date().getFullYear() - 30} />
<AnimatePresence mode="wait">
...
{activeTab === 'astro'        && <><AstroDisplay insight={insight} />{synthesis && <><SynthesisAstroSections synthesis={synthesis} insight={insight} /><SynthesisAstroMoreSections synthesis={synthesis} /></>}</>}
{activeTab === 'numerology'   && <><NumerologyDisplay numerology={numerology} />{synthesis && numerology && <SynthesisNumerologySections synthesis={synthesis} numerology={numerology} />}<NumberMeaningsSection /></>}
{activeTab === 'new-astro'    && <NewAstroSignDetails sign={insight.new_astrology_sign} signData={insight.signData} />}
{activeTab === 'psychomatrix' && <><PsychomatrixDisplay day={numerology.birthDay} month={numerology.birthMonth} year={numerology.birthYear} gender={insight.gender} name={insight.name} />{synthesis && numerology && <SynthesisPsychomatrixSections synthesis={synthesis} numerology={numerology} />}{synthesis && <SynthesisCellPairsSections synthesis={synthesis} />}{synthesis && <SynthesisFourPlanesSections synthesis={synthesis} />}</>}
...
<FloatingNavigation onReset={onReset} onHistoryOpen={onHistoryOpen} insight={insight} numerology={numerology} />
</>
);
}