...
function contentOpf(name: string, chapters: { id: string; title: string }[]): string {
...
function navXhtml(name: string, chapters: { id: string; title: string }[]): string {
  const items = chapters.map(c =>
    `<li><a href="${c.id}.xhtml">${esc(c.title)}</a></li>`
  ).join('\n      ');
  return xhtmlWrap('Navigation', `<nav epub:type="toc" xmlns:epub="http://www.idpf.org/2007/ops">
...
function buildCover(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
  const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  const bday = `${numerology.birthDay} ${monthNames[numerology.birthMonth - 1]} ${numerology.birthYear}`;
  return xhtmlWrap('Cover', `<div class="cover">
...
function buildChapter1(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
  const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];

  // Core numbers chip table
  const chips = [
    { n: numerology.psycheNum,                 l: 'Psychic Number',   note: 'Ruling self-expression' },
    { n: numerology.destinyNum,                l: 'Destiny Number',   note: 'Life purpose' },
    { n: synthesis.lifePath?.number ?? '—',    l: 'Life Path',        note: synthesis.lifePath?.title ?? '' },
    { n: synthesis.maturityNumber?.number ?? '—', l: 'Maturity Number', note: 'Emerges mid-life' },
    { n: numerology.karmicFateNum ?? '—',      l: 'Karmic Fate',      note: numerology.karmicFateNum ? 'Karmic obligation present' : 'No obligation' },
    { n: numerology.kuaNum,                    l: 'Kua Number',       note: `${numerology.kuaAttributes?.element ?? ''} group` },
  ];
  ...
  const extTable = kvTable([
    ...
    ['Personal Year (current)', synthesis.personalYearAnalysis ? (synthesis.personalYearAnalysis.rawYear === synthesis.personalYearAnalysis.year ? synthesis.personalYearAnalysis.year : `${synthesis.personalYearAnalysis.rawYear}/${synthesis.personalYearAnalysis.year}`) : '—'],
  ...
  if (synthesis.masterNumbers?.length > 0) {
    masterHtml = synthesis.masterNumbers.map(mn =>
  ...
  if (numerology.loShuGrid) {
    const LOSHU_LABEL: Record<number, string> = {1:'SW',2:'S',3:'SE',4:'W',5:'C',6:'E',7:'NW',8:'N',9:'NE'};
    const grid = numerology.loShuGrid;
    loShuHtml = `<table class="grid">
<tr>${[0,1,2].map(col => { const v = grid[0]?.[col]; return `<td class="${v?'':'empty'}">${esc(v || '·')}</td>`; }).join('')}</tr>
<tr>${[0,1,2].map(col => { const v = grid[1]?.[col]; return `<td class="${v?'':'empty'}">${esc(v || '·')}</td>`; }).join('')}</tr>
<tr>${[0,1,2].map(col => { const v = grid[2]?.[col]; return `<td class="${v?'':'empty'}">${esc(v || '·')}</td>`; }).join('')}</tr>
</table>`;
  }
  ...
  const alertsHtml = (() => {
    ...
    const parts: string[] = [];
    if (py === 4) parts.push(`<div class="alert warning"><strong>Personal Year 4 — Foundation Cycle:</strong> Consolidation year. Demands patience, structural work, and foundation-laying. Avoid impulsive expansion.</div>`);
    if (py === 7) parts.push(`<div class="alert warning"><strong>Personal Year 7 — Retreat &amp; Sacrifice:</strong> An inward spiritual year. Avoid major financial or relational decisions.</div>`);
    if (py === 9) parts.push(`<div class="alert"><strong>Personal Year 9 — Great Completion:</strong> Endings, release, and culmination. Radical release clears the path.</div>`);
    return parts.join('\n');
  })();
...
function buildChapter2(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
  // Western sign
  const westSection = section('Western Zodiac Sign', kvTable([
  ...
  const chineseBody = [
    ...
    sd?.compatibilities ? `<h3>Compatibilities</h3>${p(sd.compatibilities)}` : '',
  ].join('');
  const chineseSection = section('Chinese Zodiac — Full Profile', chineseBody);

  // Decan
  const decanSection = synthesis.decan ? section(`Decan ${synthesis.decan.decanNumber} — ${synthesis.decan.title}`, [
    kvTable([
  ...
  // Birth day of week
  const bdowSection = synthesis.birthDayOfWeek ? section(`Born on ${synthesis.birthDayOfWeek.dayName}`, [
    kvTable([
  ...
  // Sexagenary
  const sexSection = synthesis.sexagenary ? section(`Sexagenary Cycle — ${synthesis.sexagenary.name}`, p(synthesis.sexagenary.interpretation)) : '';

  // Heavenly Stem
  const stemSection = synthesis.heavenlyStem ? section(`Heavenly Stem — ${synthesis.heavenlyStem.stem}`, [
    kvTable([
  ...
  // Earthly Branch
  const branchSection = synthesis.earthlyBranch ? section(`Earthly Branch — ${synthesis.earthlyBranch.animal} (${synthesis.earthlyBranch.animalChinese})`, [
    kvTable([
  ...
  // Double Animal
  const doubleSection = (synthesis.doubleAnimal?.isDouble && synthesis.doubleAnimal.interpretation)
    ? section(`Double ${synthesis.doubleAnimal.animal} Influence`, p(synthesis.doubleAnimal.interpretation))
    : '';

  // Zodiac Relationships
  const zodRelSection = synthesis.zodiacRelationships ? section(`Zodiac Relationships — ${synthesis.zodiacRelationships.animal}`, [
    kvTable([
      ['Secret Friend', synthesis.zodiacRelationships.secretFriend],
      ['Astrological Enemy', synthesis.zodiacRelationships.astrologicalEnemy],
  ...
  const kuaSection = kua ? section(`Kua Number ${numerology.kuaNum} — Feng Shui Blueprint`, [
    kvTable([
      ['Element', kua.element ?? '—'],
      ['Group', kua.group ?? '—'],
      ['Trigram', kua.trigram ?? '—'],
      ['Season', kua.season ?? '—'],
      ['Lucky Colours', Array.isArray(kua.lucky_colours) ? kua.lucky_colours.join(', ') : (kua.colors ?? '—')],
    ]),
    kua.directions && Object.keys(kua.directions).length > 0
      ? `<h3>Power Directions</h3><table class="score"><tr><th>Direction</th><th>Meaning</th></tr>${Object.entries(kua.directions).map(([d, m]) => `<tr><td><strong>${esc(d)}</strong></td><td>${esc(m)}</td></tr>`).join('')}</table>`
      : '',
  ].join('')) : '';

  // Chinese futures
  const futures = insight.zodiacData?.futures;
  const futuresSection = (futures && Object.keys(futures).length > 0)
    ? section('Elemental Year Fortunes', `<table class="score"><tr><th>Year</th><th>Element</th><th>Prediction</th></tr>${Object.entries(futures).map(([yr, f]: [string, any]) => `<tr><td>${esc(f.year || yr)}</td><td>${esc(f.element ?? '')}</td><td>${esc(f.prediction ?? '')}</td></tr>`).join('')}</table>`)
    : '';

  // Chinese elements
  const elements = insight.zodiacData?.elements;
  const elemSection = (elements && Object.keys(elements).length > 0)
    ? section('Chinese Element Profiles', Object.entries(elements).map(([el, desc]) => `<h3>${esc(el)}</h3>${p(String(desc))}`).join(''))
    : '';

  // Compatibilities table
  const compat = insight.zodiacData?.compatibilities;
  const compatSection = (compat && Object.keys(compat).length > 0)
    ? section('Chinese Zodiac Compatibilities', `<table class="score"><tr><th>Animal</th><th>Compatibility</th></tr>${Object.entries(compat).map(([animal, desc]) => `<tr><td><strong>${esc(animal)}</strong></td><td>${esc(desc)}</td></tr>`).join('')}</table>`)
    : '';
...
function buildChapter3(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
  const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  ...
  const layer1 = section('Layer 1 — Psychic Essence (Vedic)', [
    numerology.psychicMeaning?.title ? `<h3>${esc(numerology.psychicMeaning.title)}</h3>` : '',
    p(numerology.psychicMeaning?.description),
  ].join(''));

  // Layer 2: Chronicler's View
  const layer2 = cheiroData ? section("Layer 2 — The Chronicler's View", [
    kvTable([
  ...
  const layer3 = monthlyData ? section('Layer 3 — Monthly Soul Chronicle', [
    kvTable([
      ['Birth Month', monthlyData.monthName],
    ]),
  ...
  const layer4 = monthlyData ? section('Layer 4 — Celestial Alignments', [
  ...
  // Destiny
  const destinySection = section(`Destiny Number — ${numerology.destinyNum}`, [
    numerology.destinyMeaning?.title ? `<h3>${esc(numerology.destinyMeaning.title)}</h3>` : '',
    p(numerology.destinyMeaning?.description),
  ].join(''));

  // Compound
  const compoundSection = [
    numerology.compoundNum ? section(`Compound Number — ${numerology.compoundNum} (unreduced)`, p(numerology.compoundMeaning ?? '')) : '',
    numerology.reducedCompoundNum ? section(`Reduced Compound — ${numerology.reducedCompoundNum}`, p(numerology.reducedCompoundMeaning ?? '')) : '',
  ].join('');

  // Karmic Fate
  const karmicSection = section(`Karmic Fate — ${numerology.karmicFateNum ?? 'None'}`, numerology.karmicFateNum
    ? p(numerology.karmicFateMeaning ?? '')
    : p(synthesis.karmicFateNullMeaning ?? 'No karmic fate number present in this configuration.'));

  // Special Trait
  const specialSection = numerology.specialTraitMeaning ? section('Special Trait', p(numerology.specialTraitMeaning)) : '';

  // Compound Personalized Insight
  const compoundInsight = synthesis.compoundPersonalizedInsight ? section('Compound Personalized Insight', p(synthesis.compoundPersonalizedInsight)) : '';

  // Life Path
  const lifePathSection = synthesis.lifePath ? section(`Life Path ${synthesis.lifePath.number} — ${synthesis.lifePath.title}`, p(synthesis.lifePath.interpretation)) : '';

  // Life Path Periods
  const lifePathPeriodsSection = (synthesis.lifePathPeriods?.periods?.length > 0)
    ? section('Life Path Periods', synthesis.lifePathPeriods.periods.map(pp =>
        `<h3>${esc(pp.period)} · Ages ${esc(pp.ages)} — Number ${esc(pp.number)} (${esc(pp.title)})</h3>${p(pp.interpretation)}`
      ).join(''))
    : '';

  // Personal Year Analysis (current)
  const pyaSection = synthesis.personalYearAnalysis ? section(`Current Personal Year ${synthesis.personalYearAnalysis.rawYear === synthesis.personalYearAnalysis.year ? synthesis.personalYearAnalysis.year : `${synthesis.personalYearAnalysis.rawYear}/${synthesis.personalYearAnalysis.year}`} — ${synthesis.personalYearAnalysis.title}`, [
    p(synthesis.personalYearAnalysis.interpretation),
    synthesis.personalYearCustomized ? `<h3>Personalized Reading</h3>${p(synthesis.personalYearCustomized)}` : '',
    synthesis.compoundEnrichedPY ? `<h3>Compound-Enriched Year Reading</h3>${p(synthesis.compoundEnrichedPY)}` : '',
  ].join('')) : '';

  // Universal Year
  const uySection = synthesis.universalYear ? section(`Universal Year ${synthesis.universalYear.yearNumber} — ${synthesis.universalYear.title}`, p(synthesis.universalYear.interpretation)) : '';

  // Personal Month
  const pmSection = synthesis.personalMonth ? section(`Personal Month ${synthesis.personalMonth.personalMonth}`, p(synthesis.personalMonth.interpretation)) : '';

  // Birth Month Analysis
  const bmaSection = synthesis.birthMonthAnalysis ? section(`Born in ${synthesis.birthMonthAnalysis.monthName} — ${synthesis.birthMonthAnalysis.title}`, p(synthesis.birthMonthAnalysis.interpretation)) : '';

  // Birth Day Analysis
  const bdaySection = synthesis.birthDayAnalysis ? section(`Birth Day ${synthesis.birthDayAnalysis.day} — ${synthesis.birthDayAnalysis.title}`, [
    kvTable([
  ...
  // Nine-Year Forecast
  const forecastSection = (numerology.personalYears && numerology.personalYears.length > 0)
    ? section('Nine-Year Personal Year Forecast', `<table class="score">
<tr><th>Year</th><th>PY</th><th>Compound</th><th>Reading</th></tr>
${numerology.personalYears.map(py => `<tr>
  ...
  // Maturity Number
  const maturitySection = synthesis.maturityNumber ? section(`Maturity Number ${synthesis.maturityNumber.number}${synthesis.maturityNumber.isMaster ? ' ★' : ''} — ${synthesis.maturityNumber.title}`, p(synthesis.maturityNumber.interpretation)) : '';

  // Master Numbers
  const masterSection = synthesis.masterNumbers?.length > 0
    ? section('Master Numbers Detected', synthesis.masterNumbers.map(mn =>
        `<h3>Master Number ${mn.number} — ${esc(mn.title)}</h3>${p(mn.interpretation)}`
      ).join(''))
    : '';

  // Four Life Challenges
  const challengesSection = (synthesis.challenges?.challenges?.length > 0)
    ? section('Four Life Challenges', synthesis.challenges.challenges.map(c =>
        `<h3>Challenge ${c.stage} — Number ${c.number}: ${esc(c.title)} (Ages ${esc(c.ages)})</h3>${p(c.interpretation)}`
      ).join(''))
    : '';

  // Karmic Debts
  const debtsSection = synthesis.karmicDebts?.length > 0
    ? section('Karmic Debts', synthesis.karmicDebts.map(kd =>
        `<h3>Debt Number ${kd.number} — ${esc(kd.title)}</h3>${kvTable([['Found in', kd.sources?.join(', ') ?? kd.foundIn]])}${p(kd.interpretation)}`
      ).join(''))
    : '';

  // Karmic Lessons
  const karLessonsSection = (synthesis.karmicLessons?.lessons?.length > 0)
    ? section('Karmic Lessons (Missing Numbers)', [
        synthesis.karmicLessons.summary ? `<h3>Summary</h3>${p(synthesis.karmicLessons.summary)}` : '',
        synthesis.karmicLessons.lessons.filter((l: any) => !l.present).map((l: any) =>
          `<h3>Missing Number ${l.number} — ${esc(l.title)}</h3>${p(l.interpretation)}`
        ).join(''),
        synthesis.karmicLessons.synthesis ? `<h3>Lessons Synthesis</h3>${p(synthesis.karmicLessons.synthesis)}` : '',
      ].join(''))
    : '';

  // Combined Absences
  const absSection = synthesis.combinedAbsences?.interpretation ? section('Combined Absence Analysis', p(synthesis.combinedAbsences.interpretation)) : '';
...
function buildChapter4(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
...
<tr>${[0,1,2].map(col => { const v = g[0]?.[col]; return `<td class="${v?'':'empty'}">${esc(v||'·')}</td>`; }).join('')}</tr>
<tr>${[0,1,2].map(col => { const v = g[1]?.[col]; return `<td class="${v?'':'empty'}">${esc(v||'·')}</td>`; }).join('')}</tr>
<tr>${[0,1,2].map(col => { const v = g[2]?.[col]; return `<td class="${v?'':'empty'}">${esc(v||'·')}</td>`; }).join('')}</tr>
</table>`;
  }

  // Repeated Number Meanings
  const repSection = (numerology.repeatedNumberMeanings && Object.keys(numerology.repeatedNumberMeanings).length > 0)
    ? section('Repeated Number Meanings', Object.entries(numerology.repeatedNumberMeanings).map(([num, meaning]) =>
  ...
  // Arrows
  const arrows = [
    ...(numerology.arrowsOfStrength || []).map((a: any) => ({ ...a, type: 'strength' })),
    ...(numerology.arrowsOfWeakness || []).map((a: any) => ({ ...a, type: 'weakness' })),
  ];
  const arrowsSection = arrows.length > 0
    ? section('Arrows of Influence', arrows.map((arrow: any) =>
...
${p(arrow.description)}
  ...
  const wnSection = section('The Four Working Numbers', `<table class="score">
...
<tr><td>IV</td><td>Qualities from Birth (II)</td><td><strong>${esc(pmx.fourth)}</strong></td></tr>
  ...
  const cellsHtml = pmx.cellReadings.map(cell => {
...
${cellDef?.meanings && cellDef.meanings.length > 1 ? `<h4>All Intensity Levels</h4><table class="score"><tr><th>Label</th><th>Interpretation</th></tr>${cellDef.meanings.map(m => `<tr><td><strong>${esc(m.label)}</strong>${m.label === cell.label ? ' ✓' : ''}</td><td>${esc(m.verbatim)}</td></tr>`).join('')}</table>` : ''}
  ...
  // Zero Analysis
  const zeroSection = (pmx.zeroAnalysis?.hasAnyZero && pmx.zeroAnalysis.interpretations?.length > 0)
    ? section('Analysis of the Great Void (Zero Cells)', pmx.zeroAnalysis.interpretations.map(txt => p(txt)).join(''))
    : '';
  ...
  const linesHtml = PSYCHOMATRIX_LINE_INTERPRETATIONS.map(line => {
    ...
    if (!level) return '';
    const typeLabel = line.type === 'row' ? 'Row' : line.type === 'column' ? 'Column' : 'Diagonal';
...
${line.alexandrovNote ? `<h4>Interpreter's Note</h4>${p(line.alexandrovNote)}` : ''}
  ...
  const activeSection = section("Active Lines — Rows, Columns &amp; Diagonals", linesHtml);

  // Dynamic Potentials
  const dynSection = dynamicResult.hasDynamicNumbers
    ? section('Dynamic Matrix Potentials (Millennium Activation)', p(dynamicResult.interpretation))
    : '';

  // Cross-Digit Interactions
  const crossSection = pmx.complementaryInsights?.length > 0
    ? section('Cross-Digit Interactions', pmx.complementaryInsights.map((ci: any) =>
        `<h3>${esc(ci.type)} — ${esc(ci.title)}</h3>${p(ci.insight)}`
      ).join(''))
    : '';

  // Cell Pairs
  const cellPairsSection = synthesis.cellPairs?.interpretation ? section('Cell Pair Interactions', p(synthesis.cellPairs.interpretation)) : '';

  // Line Intersections
  const lineIntSection = synthesis.lineIntersections?.interpretation ? section('Line Intersection Analysis', p(synthesis.lineIntersections.interpretation)) : '';
...
function buildChapter5(insight: AstroInsightOutput, numerology: NumerologyData, synthesis: ReturnType<typeof computeSynthesis>): string {
  const pmx = calculatePsychomatrix(numerology.birthDay, numerology.birthMonth, numerology.birthYear);
  ...
  const planesSection = planes ? section('Four Planes of Expression', [
    `<table class="score">
<tr><th>Plane</th><th>Score</th><th>Max</th><th>%</th><th>Level</th><th>Cells</th></tr>
${[planes.mental, planes.physical, planes.emotional, planes.intuitive].map(pl =>
  `<tr><td><strong>${esc(pl.name)}</strong></td><td>${esc(pl.score)}</td><td>${esc(pl.maxScore)}</td><td>${esc(pl.percentage)}%</td><td>${esc(pl.level)}</td><td>${pl.cells.join(', ')}</td></tr>`
    ...
    planes.synthesis ? `<h3>Synthesis Interpretation</h3>${p(planes.synthesis)}` : '',
    [planes.mental, planes.physical, planes.emotional, planes.intuitive].map(pl =>
  ...
  const rankSection = dominance ? section('Psychic Architecture — Priority Ranking', [
    `<table class="score">
<tr><th>Rank</th><th>Force</th><th>Score</th><th>Description</th></tr>
${dominance.rankedElements.map((entry: any, i: number) =>
  `<tr><td>${i + 1}</td><td><strong>${esc(entry.element)}</strong></td><td>${esc(entry.score)}</td><td>${esc(entry.description)}</td></tr>`
).join('')}
</table>`,
    dominance.top3?.length > 0 ? dominance.top3.map((entry: any) =>
  ...
  const domainsSection = domainRows ? section('Life-Domain Grid Profile', domainRows) : '';

  // Creative Tensions (Contradictions)
  const tensionsSection = contradictions.length > 0
    ? section(`Creative Tensions (${contradictions.length})`, contradictions.map((c, idx) =>
...
${esc(c.resolution)}
  ...
  const recsSection = recommendations.length > 0
    ? section(`Consultant Recommendations (${recommendations.length})`, recommendations.map((r, idx) =>
        `<div class="rec-block">
...
<div class="rec-practice"><strong>30-Day Practice:</strong><br/>${esc(r.practice)}</div>
  ...
  const cellPairSection = cellPairPatterns.length > 0
    ? section('Cross-Cell Pattern Details', `<table class="score">
...
${cellPairPatterns.map((p: any) => `<tr><td><strong>${esc(p.name)}</strong></td><td>${p.involvedCells?.join(', ') ?? ''}</td><td>${esc(p.polarity)}</td><td>${esc(p.interpretation)}</td></tr>`).join('')}
  ...
  const lineIntTableSection = lineIntPatterns.length > 0
    ? section('Line Intersection Synthesis', `<table class="score">
...
${lineIntPatterns.map((p: any) => `<tr><td><strong>${esc(p.name)}</strong></td><td>${esc(p.condition)}</td><td>${esc(p.interpretation)}</td></tr>`).join('')}
  ...
  const maturityDeepSection = synthesis.maturityNumber ? section(`Maturity Number ${synthesis.maturityNumber.number} — Deep Synthesis`, p(synthesis.maturityNumber.interpretation)) : '';

  // Compound Personalized
  const compDeepSection = synthesis.compoundPersonalizedInsight ? section('Compound Personalized Insight', p(synthesis.compoundPersonalizedInsight)) : '';

  // Personal Year Customized (deep)
  const pyCustomDeepSection = synthesis.personalYearCustomized ? section('Personal Year — Customized Reading', [p(synthesis.personalYearCustomized), synthesis.compoundEnrichedPY ? `<h3>Compound-Enriched Year</h3>${p(synthesis.compoundEnrichedPY)}` : ''].join('')) : '';
...
export async function generateEpub(insight: AstroInsightOutput, numerology: NumerologyData) {
  const zip = new JSZip();

  const synthesis = computeSynthesis(numerology, insight);

  const chapters = [
    { id: 'cover',  title: 'Cover & Contents',                  html: buildCover(insight, numerology, synthesis) },
    { id: 'ch-01',  title: 'Overview & Core Numbers',           html: buildChapter1(insight, numerology, synthesis) },
    { id: 'ch-02',  title: 'Cosmic Map — Astrology',            html: buildChapter2(insight, numerology, synthesis) },
    { id: 'ch-03',  title: 'Numerology',                        html: buildChapter3(insight, numerology, synthesis) },
    { id: 'ch-04',  title: 'Chambers — Psychomatrix & Lo Shu',  html: buildChapter4(insight, numerology, synthesis) },
    { id: 'ch-05',  title: 'Synthesis — Deep Integration',      html: buildChapter5(insight, numerology, synthesis) },
  ];
...
export function EpubExportButton({ insight, numerology }: EpubExportButtonProps) {
  const [loading, setLoading] = useState(false);

  const handleExport = async () => {
    setLoading(true);
    try { await generateEpub(insight, numerology); }
    catch (err) { console.error('EPUB export failed', err); }
  ...
  return (
    <button
      ...
      style={{
        ...
        border: '1px solid rgba(52,211,153,0.45)', borderRadius: '50%',
        padding: 0,
